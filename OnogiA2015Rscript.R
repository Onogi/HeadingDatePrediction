...
PlotPredictions <- function (Name, Prediction, Heading, Height, Width, Xlim, Ylim){
  ...
  tiff(paste(Name,".tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  plot(Heading[1:(Ne*Nline)],Prediction[1:(Ne*Nline)],
       pch=c(rep(0,Nline),rep(1,Nline),rep(2,Nline),rep(3,Nline),rep(4,Nline),rep(5,Nline),rep(6,Nline),rep(7,Nline),rep(8,Nline)),       
       main="",xlab="",ylab="",xlim=Xlim,ylim=Ylim)
  lines(c(0,300),c(0,300),lty=2)
  v<-coef(lm(Prediction[1:(Ne*Nline)]~Heading[1:(Ne*Nline)]))
  lines(c(0,300),c(v[1],v[1]+300*v[2]),lty=3)
  dev.off()
}
...
#variance explained
c(apply(O.fitting.EBL[[1]],2,var)/apply(O.fitting[[1]],2,var))[c(1,3,5)]
0.6033069 0.5130999 0.5647948

#plot marker effects
#g
tiff("O.fitting.coef.g.tif",unit="cm",height=3,width=17,res=600)
par(mar=c(1,2,1,0.5))
par(cex=0.8)
...
dev.off()
#a
tiff("O.fitting.coef.a.tif",unit="cm",height=3,width=17,res=600)
par(mar=c(1,2,1,0.5))
par(cex=0.8)
...
dev.off()
#b
tiff("O.fitting.coef.b.tif",unit="cm",height=3,width=17,res=600)
par(mar=c(1,2,1,0.5))
par(cex=0.8)
...
round(sqrt(DTHsqdif[c(1,7,8,9,5,2,4,6,3),c(1,7,8,9,5,2,4,6,3)]),1)#Table 3
[,1] [,2] [,3] [,4] [,5] [,6] [,7] [,8] [,9]
...
round(cor(DTH[,c(1,7,8,9,5,2,4,6,3)],use="pairwise.complete.obs"),2) #Table 3
...
Ilog.fitting.sample[[2]]<-as.matrix(read.table("BIL.fitting.Ilog_fitting_sampledg.txt"))
Ilog.fitting.sample[[3]]<-as.matrix(read.table("BIL.fitting.Ilog_fitting_sampleda.txt"))
Ilog.fitting.sample[[4]]<-as.matrix(read.table("BIL.fitting.Ilog_fitting_sampledb.txt"))
...
Ilog.fitting.EBL[[1]][Ilog.fitting.EBL[[1]]<0]<-0
for(env in 1:Ne){
  for(line in 1:Nline){
    Ilog.fitting.EBL[[2]][line,2*env-1]<-
      Predict(Temp2[,env],Photo2[,env],Ilog.fitting.EBL[[1]][line,1],Ilog.fitting.EBL[[1]][line,3],Ilog.fitting.EBL[[1]][line,5],Emergence[line,env])    
  } 
}
...
#variance explained
c(apply(Ilog.fitting.EBL[[1]],2,var)/apply(Ilog.fitting[[1]],2,var))[c(1,3,5)]
0.5613970 0.6427125 0.4349178
...
for(foldenv in 1:Ne){
  for(para in c(1,3,5)){
    cat(foldenv,para,"\n")
    temp<-log(Ilog.8env[[foldenv]][[1]][,para])
    v<-vigor(temp,Geno,"EBL",c(0.1,0.1,1,0.1),Threshold=9,Printinfo=F)
    Ilog.8env.EBL[[foldenv]][,para]<-exp(colSums(t(Geno)*v$Beta)+v$Alpha)
  }
  for(line in 1:Nline){
    Ilog.8env.EBL[[Ne+1]][line,2*foldenv-1]<-
      Predict(Temp2[,foldenv],Photo2[,foldenv],Ilog.8env.EBL[[foldenv]][line,1],Ilog.8env.EBL[[foldenv]][line,3],Ilog.8env.EBL[[foldenv]][line,5],Emergence[line,foldenv])    
  }
}
...
for(foldline in 1:Nline){
  g<-Ilog.175line.EBL[[1]][foldline,1];if(g<0)g<-0  
  a<-Ilog.175line.EBL[[1]][foldline,3];if(a<0)a<-0
  b<-Ilog.175line.EBL[[1]][foldline,5];if(b<0)b<-0
  for(env in 1:Ne){
    Ilog.175line.EBL[[2]][foldline,2*env-1]<-Predict(Temp2[,env],Photo2[,env],g,a,b,Emergence[foldline,env])   
  }        
}
...
for(foldenv in 1:Ne){
  for(foldline in 1:Nline){
    cat(foldenv,foldline,"\n")
    g<-Ilog.8env.175line.EBL[[foldenv]][foldline,1];if(g<0)g<-0
    a<-Ilog.8env.175line.EBL[[foldenv]][foldline,3];if(a<0)a<-0
    b<-Ilog.8env.175line.EBL[[foldenv]][foldline,5];if(b<0)b<-0
    Ilog.8env.175line.EBL[[Ne+1]][foldline,2*foldenv-1]<-Predict(Temp2[,foldenv],Photo2[,foldenv],g,a,b,Emergence[foldline,foldenv])
  }
}
...
var(exp(Geno%*%matrix(EBL.fitting.coef$g[-1],nc=1)+EBL.fitting.coef$g[1]))/var(EBL.fitting [[1]][,1])
0.7245945
var(exp(Geno%*%matrix(EBL.fitting.coef$a[-1],nc=1)+EBL.fitting.coef$a[1]))/var(EBL.fitting [[1]][,3])
0.8036327
var(exp(Geno%*%matrix(EBL.fitting.coef$b[-1],nc=1)+EBL.fitting.coef$b[1]))/var(EBL.fitting [[1]][,5])
0.8182454
...
for(i in 1:4){ #four analyses were conducted with different initial values
  ...
  EBL.fitting.rep[[i]]<-data.frame(g=apply(as.matrix(read.table("BIL.fitting.EBL_fitting_sampledg.txt")),2,mean),
                                   gsd=apply(as.matrix(read.table("BIL.fitting.EBL_fitting_sampledg.txt")),2,sd),
                                   a=apply(as.matrix(read.table("BIL.fitting.EBL_fitting_sampleda.txt")),2,mean),
                                   asd=apply(as.matrix(read.table("BIL.fitting.EBL_fitting_sampleda.txt")),2,sd),
                                   b=apply(as.matrix(read.table("BIL.fitting.EBL_fitting_sampledb.txt")),2,mean),
                                   bsd=apply(as.matrix(read.table("BIL.fitting.EBL_fitting_sampledb.txt")),2,sd))
  setwd("../../")
}
...
T-Bay     17.208693 0.783319 
IM        6.5428450 0.9301512

PlotPredictions("8env.O",O.8env[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.Ilog",Ilog.8env[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.O.EBL",O.8env.EBL[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.Ilog.EBL",Ilog.8env.EBL[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.EBL",EBL.8env[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
...
PlotPredictions("175line.D.EBL",Direct.EBL.175line,Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("175line.O.EBL",O.175line.EBL$DH[,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("175line.Ilog.EBL",Ilog.175line.EBL[[2]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("175line.EBL",EBL.175line[[Nline+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
...
PlotPredictions("8env.175line.D.EBL",Direct.EBL.8env.175line,Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.175line.O.EBL",O.8env.175line.EBL[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.175line.Ilog.EBL",Ilog.8env.175line.EBL[[Ne+1]][,Odds],Heading,5.4,5.4,c(50,180),c(50,180))
PlotPredictions("8env.175line.EBL",EBL.8env.175line[,Odds],Heading,5.4,5.4,c(50,180),c(50,180))


#Comparison of estimated parameter values among methods###################################################################################################
#g
Gsummary<-round(cor(cbind(O.fitting[[1]]$g,Ilog.fitting[[1]]$g,O.fitting.EBL[[1]][,1],Ilog.fitting.EBL[[1]][,1],EBL.fitting[[1]]$g)),2)
Gsummary[upper.tri(Gsummary,diag=F)]<-
  round(cor(cbind(O.fitting[[1]]$g,Ilog.fitting[[1]]$g,O.fitting.EBL[[1]][,1],Ilog.fitting.EBL[[1]][,1],EBL.fitting[[1]]$g),
            method="spearman"),2)[upper.tri(Gsummary,diag=F)]
Gsummary
[,1] [,2] [,3] [,4] [,5]
[1,] 1.00 0.91 0.82 0.75 0.88
[2,] 0.93 1.00 0.81 0.80 0.93
[3,] 0.86 0.79 1.00 0.95 0.90
[4,] 0.81 0.84 0.94 1.00 0.89
[5,] 0.92 0.95 0.90 0.90 1.00
round(c(mean(O.fitting[[1]]$g),mean(Ilog.fitting[[1]]$g),mean(O.fitting.EBL[[1]][,1]),mean(Ilog.fitting.EBL[[1]][,1]),mean(EBL.fitting[[1]]$g)),1)
60.6 64.1 60.6 64.1 62.9
round(c(sd  (O.fitting[[1]]$g),sd  (Ilog.fitting[[1]]$g),sd  (O.fitting.EBL[[1]][,1]),sd  (Ilog.fitting.EBL[[1]][,1]),sd  (EBL.fitting[[1]]$g)),1)
8.5 6.2 6.6 4.6 6.9
#a
asummary<-round(cor(cbind(O.fitting[[1]]$a,Ilog.fitting[[1]]$a,O.fitting.EBL[[1]][,3],Ilog.fitting.EBL[[1]][,3],EBL.fitting[[1]]$a)),2)
asummary[upper.tri(asummary,diag=F)]<-
  round(cor(cbind(O.fitting[[1]]$a,Ilog.fitting[[1]]$a,O.fitting.EBL[[1]][,3],Ilog.fitting.EBL[[1]][,3],EBL.fitting[[1]]$a),
            method="spearman"),2)[upper.tri(asummary,diag=F)]
asummary
[,1] [,2] [,3] [,4] [,5]
[1,] 1.00 0.81 0.83 0.73 0.76
[2,] 0.78 1.00 0.82 0.89 0.94
[3,] 0.83 0.79 1.00 0.89 0.86
[4,] 0.70 0.87 0.89 1.00 0.94
[5,] 0.75 0.94 0.85 0.93 1.00
round(c(mean(O.fitting[[1]]$a),mean(Ilog.fitting[[1]]$a),mean(O.fitting.EBL[[1]][,3]),mean(Ilog.fitting.EBL[[1]][,3]),mean(EBL.fitting[[1]]$a)),2)
1.30 1.64 1.30 1.64 1.50
round(c(sd  (O.fitting[[1]]$a),sd  (Ilog.fitting[[1]]$a),sd  (O.fitting.EBL[[1]][,3]),sd  (Ilog.fitting.EBL[[1]][,3]),sd  (EBL.fitting[[1]]$a)),2)
0.41 0.50 0.30 0.40 0.38
#b
bsummary<-round(cor(cbind(O.fitting[[1]]$b,Ilog.fitting[[1]]$b,O.fitting.EBL[[1]][,5],Ilog.fitting.EBL[[1]][,5],EBL.fitting[[1]]$b)),2)
bsummary[upper.tri(bsummary,diag=F)]<-
  round(cor(cbind(O.fitting[[1]]$b,Ilog.fitting[[1]]$b,O.fitting.EBL[[1]][,5],Ilog.fitting.EBL[[1]][,5],EBL.fitting[[1]]$b),
            method="spearman"),2)[upper.tri(bsummary,diag=F)]
bsummary
[,1] [,2] [,3] [,4] [,5]
[1,] 1.00 0.95 0.90 0.81 0.92
[2,] 0.89 1.00 0.90 0.84 0.94
[3,] 0.87 0.75 1.00 0.94 0.96
[4,] 0.81 0.80 0.92 1.00 0.91
[5,] 0.92 0.90 0.92 0.89 1.00
round(c(mean(O.fitting[[1]]$b),mean(Ilog.fitting[[1]]$b),mean(O.fitting.EBL[[1]][,5]),mean(Ilog.fitting.EBL[[1]][,5]),mean(EBL.fitting[[1]]$b)),2)
5.02 3.02 5.07 3.07 3.98
round(c(sd  (O.fitting[[1]]$b),sd  (Ilog.fitting[[1]]$b),sd  (O.fitting.EBL[[1]][,5]),sd  (Ilog.fitting.EBL[[1]][,5]),sd  (EBL.fitting[[1]]$b)),2)
3.68 2.77 2.77 1.82 2.83

#biplots
Gsummary.plot<-data.frame(cbind(O.fitting[[1]]$g,Ilog.fitting[[1]]$g,O.fitting.EBL[[1]][,1],Ilog.fitting.EBL[[1]][,1],EBL.fitting[[1]]$g))
colnames(Gsummary.plot)<-c("C-Nel","C-Bay","T-Nel","T-Bay","IM")
tiff("Gsummary.biplot.tif",unit="cm",width=17,height=17,res=400)
par(cex=0.5)
par(mar=c(2,1,1,0.5))
plot(Gsummary.plot,main="")
dev.off()

asummary.plot<-data.frame(cbind(O.fitting[[1]]$a,Ilog.fitting[[1]]$a,O.fitting.EBL[[1]][,3],Ilog.fitting.EBL[[1]][,3],EBL.fitting[[1]]$a))
colnames(asummary.plot)<-c("C-Nel","C-Bay","T-Nel","T-Bay","IM")
tiff("asummary.biplot.tif",unit="cm",width=17,height=17,res=400)
par(cex=0.5)
par(mar=c(2,1,1,0.5))
plot(asummary.plot,main="")
dev.off()

bsummary.plot<-data.frame(cbind(O.fitting[[1]]$b,Ilog.fitting[[1]]$b,O.fitting.EBL[[1]][,5],Ilog.fitting.EBL[[1]][,5],EBL.fitting[[1]]$b))
colnames(bsummary.plot)<-c("C-Nel","C-Bay","T-Nel","T-Bay","IM")
tiff("bsummary.biplot.tif",unit="cm",width=17,height=17,res=400)
par(cex=0.5)
par(mar=c(2,1,1,0.5))
plot(bsummary.plot,main="")
dev.off()
...
for(para in 1:3){
  Para<-c("G","a","b")[para]
  Xlim<-range(c(O.fitting[[1]][,2*para-1],
              Ilog.fitting[[1]][,2*para-1],
              O.fitting.EBL[[1]][,2*para-1],
              Ilog.fitting.EBL[[1]][,2*para-1],
              EBL.fitting[[1]][,2*para-1]))
  Xlim<-Xlim*1.05  
  if(para==1){Xlim[1]<-floor(Xlim[1])}else{Xlim[1]<-0}
  if(para==1){Breaks=seq(0,300,2)}else{if(para==2){Breaks=seq(0,20,0.1)}else{Breaks=seq(0,20,0.5)}}
  
  tiff(paste(Para,"_O.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(O.fitting[[1]][,2*para-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off() 
  tiff(paste(Para,"_Ilog.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(Ilog.fitting[[1]][,2*para-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
  tiff(paste(Para,"_O.EBL.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(O.fitting.EBL[[1]][,2*para-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off() 
  tiff(paste(Para,"_Ilog.EBL.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(Ilog.fitting.EBL[[1]][,2*para-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
  tiff(paste(Para,"_EBL.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(EBL.fitting[[1]][,2*para-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
}

#proportion of lines with b<=0.5
length(c(1:176)[O.fitting[[1]]$b<=0.5])/176
length(c(1:176)[Ilog.fitting[[1]]$b<=0.5])/176
length(c(1:176)[O.fitting.EBL[[1]][,5]<=0.5])/176
length(c(1:176)[Ilog.fitting.EBL[[1]][,5]<=0.5])/176
length(c(1:176)[EBL.fitting[[1]]$b<=0.5])/176
...
#Distributions of fitted values
Mar<-c(2,2,0.5,0.5)
Cex<-0.5
Width<-3
Height<-3
for(env in 1:Ne){
  Para<-Environments[env]
  Xlim<-range(c(O.fitting[[2]][,2*env-1],
                O.fitting.EBL[[2]][,2*env-1],
                Ilog.fitting[[2]][,2*env-1],
                Ilog.fitting.EBL[[2]][,2*env-1],
                EBL.fitting[[2]][,2*env-1]))+c(-1,1)
  Breaks<-seq(0,300,2)
  tiff(paste(Para,"_O.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(O.fitting[[2]][,2*env-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
  tiff(paste(Para,"_Ilog.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(Ilog.fitting[[2]][,2*env-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
  tiff(paste(Para,"_O.EBL.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(O.fitting.EBL[[2]][,2*env-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()  
  tiff(paste(Para,"_Ilog.EBL.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(Ilog.fitting.EBL[[2]][,2*env-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
  tiff(paste(Para,"_EBL.tif",sep=""),unit="cm",width=Width,height=Height,res=600)
  par(mar=Mar)
  par(cex=Cex)
  hist(EBL.fitting[[2]][,2*env-1],main="", xlab="",ylab="",xlim=Xlim,breaks=Breaks)
  dev.off()
}

#Mean and SD of fitted values
for(env in 1:Ne){
  cat(Environments[env],"\n")
  cat(round(mean(O.fitting[[2]][,2*env-1]),1),       round(sd(O.fitting[[2]][,2*env-1]),1),"\n")
  cat(round(mean(Ilog.fitting[[2]][,2*env-1]),1),    round(sd(Ilog.fitting[[2]][,2*env-1]),1),"\n")   
  cat(round(mean(O.fitting.EBL[[2]][,2*env-1]),1),   round(sd(O.fitting.EBL[[2]][,2*env-1]),1),"\n")  
  cat(round(mean(Ilog.fitting.EBL[[2]][,2*env-1]),1),round(sd(Ilog.fitting.EBL[[2]][,2*env-1]),1),"\n")
  cat(round(mean(EBL.fitting[[2]][,2*env-1]),1),     round(sd(EBL.fitting[[2]][,2*env-1]),1),"\n") 
}
#Tsukuba2008E
120.1 14.9 
121.6 15 
